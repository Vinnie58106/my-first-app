<?php
require_once 'config.php';

if (!isset($_SESSION['user_id'])) {
    echo json_encode(['success' => false, 'message' => 'Please login first']);
    exit();
}

$data = json_decode(file_get_contents('php://input'), true);
$course_id = $data['course_id'] ?? 0;
$user_id = $_SESSION['user_id'];

// Check if already registered
$stmt = $pdo->prepare("SELECT id FROM registrations WHERE user_id = ? AND course_id = ? AND status = 'active'");
$stmt->execute([$user_id, $course_id]);
if ($stmt->fetch()) {
    echo json_encode(['success' => false, 'message' => 'Already registered for this course']);
    exit();
}

// Check capacity
$stmt = $pdo->prepare("SELECT capacity, enrolled FROM courses WHERE id = ?");
$stmt->execute([$course_id]);
$course = $stmt->fetch();

if (!$course) {
    echo json_encode(['success' => false, 'message' => 'Course not found']);
    exit();
}

if ($course['enrolled'] >= $course['capacity']) {
    echo json_encode(['success' => false, 'message' => 'Course is full']);
    exit();
}

// Register
$pdo->beginTransaction();
$stmt = $pdo->prepare("INSERT INTO registrations (user_id, course_id) VALUES (?, ?)");
$stmt->execute([$user_id, $course_id]);

$stmt = $pdo->prepare("UPDATE courses SET enrolled = enrolled + 1 WHERE id = ?");
$stmt->execute([$course_id]);
$pdo->commit();

echo json_encode(['success' => true, 'message' => 'Successfully registered!']);
?>